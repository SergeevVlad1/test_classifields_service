...
export const createFormField = <T extends BaseFieldProps>(
  ...
  return ({ name, type, value, ...props }: T & Record<string, any>) => {
    ...
    return (
      <div
        className={`form-field-container ${meta.error && meta.touched ? "has-error" : ""}`}
      >
        <Component {...field} {...props} {...additionalProps} type={type} />
        {meta.error && meta.touched && (
          <div className="error-message-bubble">{meta.error}</div>
        )}
      </div>
    );
  };
